...
class WalletWebRoutesTests(unittest.TestCase):
    ...
    def test_player_wallet_page_requires_identity_and_renders_history(self) -> None:
        ...
        self.assertIn("Demo Wallet", body)
        self.assertIn(self.owner_id, body)
        self.assertIn("Available balance", body)
        self.assertNotIn("Wallet admin workspace", body)
        self.assertIn("Demo only", body)
        self.assertIn("Switch demo player or filter activity", body)
        self.assertIn("Back to Trader", body)
        self.assertNotIn("Net ledger total", body)
