...
| BOT-04 | Moderation bot | Warnings, staff actions, roles, logs, permission checks | Design inventory completed in Slice 5 (`docs/MODERATION_COMMUNITY_RECOVERY_PLAN.md`) | `C:\DXEMB\discord_bot\src\cogs\moderation.py`; `C:\DXEMB\core\services\moderation_service.py`; `C:\DXEMB\core\services\audit_service.py` | Needs recovery | Recover in split modules | Duplicate-generation moderation file confirmed; split/adapt before enabling any commands |
...
- Next recommended sprint: moderation/tickets implementation in controlled, test-first slices.
