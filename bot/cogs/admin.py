...
Fixed:
  - confirm_delivery / refund_escrow modals pass economy_service (bot.economy)
  - expire_loop passes economy_service
"""
...
class ConfirmDeliveryModal(discord.ui.Modal, title='Confirm Market Delivery'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        # Fixed: pass economy_service
        result = await interaction.client.market_service.confirm_delivery(
            interaction.user.id, str(self.listing_id), interaction.client.economy
        )
        await interaction.response.send_message(
...
class RefundEscrowModal(discord.ui.Modal, title='Refund Escrow to Buyer'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        # Fixed: pass economy_service
        result = await interaction.client.market_service.refund_escrow(
            interaction.user.id, str(self.listing_id), interaction.client.economy
        )
        await interaction.response.send_message(
            f'\u2705 {result["message"]}' if result['success'] else f'\u274c {result["message"]}', ephemeral=True
...
class AdminPanelView(discord.ui.View):
    ...
    @discord.ui.button(label='\U0001f4e6 Create Bundle', style=discord.ButtonStyle.blurple, custom_id='admin:create_bundle', row=0)
    async def create_bundle(self, interaction, button):
        if not await self._admin_check(interaction): return
        await interaction.response.send_modal(AddBundleModal())

    @discord.ui.button(label='\U0001f504 Toggle Item', style=discord.ButtonStyle.grey, custom_id='admin:toggle_item', row=0)
    async def toggle_item(self, interaction, button):
        if not await self._admin_check(interaction): return
        await interaction.response.send_modal(ToggleItemModal())

    @discord.ui.button(label='\U0001f4b0 Adjust Balance', style=discord.ButtonStyle.red, custom_id='admin:adjust_balance', row=1)
    async def adjust_balance(self, interaction, button):
        if not await self._admin_check(interaction): return
    ...
    @discord.ui.button(label='\U0001f4b8 Refund Escrow', style=discord.ButtonStyle.red, custom_id='admin:refund_escrow', row=1)
    async def refund_escrow(self, interaction, button):
        if not await self._admin_check(interaction): return
        await interaction.response.send_modal(RefundEscrowModal())

    @discord.ui.button(label='\U0001f4cb View Disputes', style=discord.ButtonStyle.grey, custom_id='admin:view_disputes', row=2)
    async def view_disputes(self, interaction, button):
        if not await self._admin_check(interaction): return
        ...
        if not listings:
            ...
            return
        embed = discord.Embed(title='\U0001f6a8 Active Disputes', color=discord.Color.red())
        for l in listings:
            embed.add_field(
    ...
    @discord.ui.button(label='\U0001f4ca Delivery Queue', style=discord.ButtonStyle.grey, custom_id='admin:delivery_queue', row=2)
    async def delivery_queue(self, interaction, button):
        if not await self._admin_check(interaction): return
        size = interaction.client.delivery_queue.queue_size
        await interaction.response.send_message(
            f'\U0001f4e6 **Delivery Queue:** {size} jobs pending\n'
            '\u26a0\ufe0f Server is **never** restarted by the bot \u2014 CE cycle handles all spawns automatically.',
            ephemeral=True
        )

    @discord.ui.button(label='\U0001f5d1\ufe0f Delete Item', style=discord.ButtonStyle.red, custom_id='admin:delete_item', row=2)
    async def delete_item_btn(self, interaction, button):
        ...
        class DeleteModal(discord.ui.Modal, title='Delete Shop Item'):
            ...
            async def on_submit(s, i: discord.Interaction):
                ...
                await i.response.send_message(f'\U0001f5d1\ufe0f Item `{s.item_id}` deleted.', ephemeral=True)
...
class AdminCog(commands.Cog):
    ...
    async def expire_loop(self):
        # Fixed: pass economy_service
        await self.bot.market_service.expire_old_listings(self.bot.economy)
    ...
    async def admin_panel(self, interaction: discord.Interaction):
        ...
        embed.add_field(name='\U0001f6d2 Shop', value='Add items, create bundles, toggle/delete listings', inline=False)
        embed.add_field(name='\U0001f3ea Market', value='Confirm deliveries, refund escrow, view disputes', inline=False)
        embed.add_field(name='\U0001f4b0 Economy', value='Adjust player balances with audit logging', inline=False)
        embed.add_field(
            name='\U0001f4e6 Deliveries',
            value='\u26a0\ufe0f Server is **never** restarted. CE cycle (~1-5 min) handles all spawns.',
            inline=False
        )
    ...
    async def ban_player(self, interaction: discord.Interaction, user: discord.Member, reason: str = ''):
        ...
        await interaction.response.send_message(f'\U0001f528 <@{user.id}> banned from trading. Reason: {reason}', ephemeral=True)
