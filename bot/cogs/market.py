...
class MarketCog(commands.Cog):
    ...
    async def market(self, interaction: discord.Interaction):
        ...
        if not listings:
            await interaction.followup.send('📭 No active listings right now. Use /list to sell something!')
            return
        embed = discord.Embed(title='🏪 Player Marketplace', color=discord.Color.blurple())
        for l in listings[:10]:
            embed.add_field(
                name=f'{l["item_display"]} x{l["quantity"]}',
                value=f'💰 {l["asking_price"]:,} credits | ID: `{l["listing_id"]}` | Seller: <@{l["seller_id"]}>',
                inline=False
            )
    ...
    async def list_item(
        ...
        if result['success']:
            ...
            embed.add_field(name='🔒 Escrow Protection', value='Buyer credits are locked until delivery is confirmed.', inline=False)
    ...
    async def buy_listing(self, interaction: discord.Interaction, listing_id: str):
        ...
        await self.bot.economy.ensure_user(interaction.user.id, interaction.user.name)
        result = await self.bot.market_service.buy_listing(interaction.user.id, listing_id)
        if result['success']:
            embed = discord.Embed(
                title='🔒 Escrow Locked — Trade In Progress',
                description=result['message'],
                color=discord.Color.orange()
            )
            embed.add_field(
                name='What happens next?',
                value='The seller will deliver your item in-game. An admin will confirm delivery and release your credits to the seller. If anything goes wrong, use /dispute.',
                inline=False
            )
            await interaction.followup.send(embed=embed)
            # Notify seller
            try:
                ...
                await seller.send(f'🛒 Someone bought your **{result["listing"]["item_display"]}** listing! Deliver it in-game and notify an admin to confirm.')
            except:
                pass
        else:
